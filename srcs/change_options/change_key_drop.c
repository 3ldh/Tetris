...
** Last update Sat Mar  5 10:03:25 2016 maud marel
*/
...
void	change_key_drop(t_tetris *tetris, char *str)
{
  ...
  if (i == my_strlen(str))
    wrong_options(str);
  if ((tetris->options->drop = malloc(sizeof(char) * my_strlen(str) - (i + 1))) == NULL)
    exit(1);
  j = 0;
  while (str[i] != '\0')
    {
...
{
  if ((tetris->options->drop = malloc(sizeof(char) * my_strlen(str) + 1)) == NULL)
    exit(1);
